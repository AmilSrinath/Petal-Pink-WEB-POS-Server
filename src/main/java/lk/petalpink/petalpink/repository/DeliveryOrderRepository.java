...
public class DeliveryOrderRepository {
    ...
    public DeliveryOrderDTO getByDeliveryId(Integer deliveryId) {
        ...
        return jdbcTemplate.queryForObject(sql, new RowMapper<DeliveryOrderDTO>() {
            @Override
            public DeliveryOrderDTO mapRow(ResultSet rs, int rowNum) throws SQLException {
                DeliveryOrderDTO dto = new DeliveryOrderDTO();
    ...
    public List<OrderDetailItemDTO> getOrderDetailsByOrderId(Integer orderId) {
        ...
        String sql =
                "SELECT " +
                        "od.order_detail_id, od.order_id, od.item_id, od.item_bar_code, " +
                        "od.unit_type_id, od.printer_type_id, od.quantity, od.per_item_price, " +
                        "od.total_discount_price, od.total_item_price, od.remark, " +
                        "i.item_name " +
                        "FROM pos_main_order_details_tb od " +
                        "JOIN pos_main_item_tb i ON od.item_id = i.item_id " +
                        "WHERE od.order_id = ? AND od.status = 1";

        return jdbcTemplate.query(sql, new RowMapper<OrderDetailItemDTO>() {
            @Override
            public OrderDetailItemDTO mapRow(ResultSet rs, int rowNum) throws SQLException {

                OrderDetailItemDTO dto = new OrderDetailItemDTO();

                dto.setItemId(rs.getObject("item_id") != null ? rs.getInt("item_id") : null);
                dto.setItemBarCode(rs.getObject("item_bar_code") != null ? rs.getInt("item_bar_code") : null);
                dto.setUnitTypeId(rs.getObject("unit_type_id") != null ? rs.getInt("unit_type_id") : null);
                dto.setPrinterTypeId(rs.getObject("printer_type_id") != null ? rs.getInt("printer_type_id") : null);
                dto.setQuantity(rs.getObject("quantity") != null ? rs.getInt("quantity") : null);
                dto.setPerItemPrice(rs.getObject("per_item_price") != null ? rs.getDouble("per_item_price") : null);
                dto.setTotalDiscountPrice(rs.getObject("total_discount_price") != null ? rs.getDouble("total_discount_price") : null);
                dto.setTotalItemPrice(rs.getObject("total_item_price") != null ? rs.getDouble("total_item_price") : null);
                dto.setRemark(rs.getString("remark"));
